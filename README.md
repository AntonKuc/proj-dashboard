...
подтягивается автоматически из Такском Кассы; 1С, Google Диск, Яндекс
Карты — ещё нет (см. `src/lib/connectors/README.md`).
...
4. В настройках проекта на Vercel добавить переменные окружения:
   ...
   `TAXCOM_PASSWORD`, `CRON_SECRET` — см. `.env.example`.
...
6. Ночной синк Такскома (`vercel.json`, `crons`) включается сам после
   деплоя — Vercel подхватывает конфиг из репозитория. Проверить, что
   сработал: `GET /api/v1/health` не про это — смотреть логи функции
   `/api/v1/sync/taxcom` в Vercel Dashboard -> Deployments -> Functions.
